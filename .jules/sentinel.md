...
## 2026-06-03 - [Temporary File Leakage on Exception]
**Vulnerability:** Temporary files created for processing (e.g., motion photo extraction) were not deleted if an exception was thrown before successful completion, leading to potential storage exhaustion and leaking incomplete sensitive media artifacts.
**Learning:** Temporary files created using `File.createTempFile` need explicit cleanup in failure paths (e.g. `finally` block or `catch` block), especially in suspend functions dealing with large media files.
**Prevention:** Always track the success status of operations involving temporary files and explicitly invoke `delete()` in the `finally` block if the operation failed. Ensure exceptions during deletion are caught and logged so they do not shadow the original exception.
