...
## 2024-05-20 - [Path Traversal in Export FileName]
**Vulnerability:** User-provided or dynamically generated file names during export operations could use multiple consecutive dots (`..` or `...`) to bypass simple `".." -> "_"` string replacements and execute path traversal attacks when saving to the file system. Control characters could also be injected to corrupt metadata or crash file systems.
**Learning:** `replace("..", "_")` only replaces exact two-dot sequences. `...` becomes `._` which is still problematic.
**Prevention:** Use regular expressions like `Regex("\\.\\.+")` to squash any sequence of two or more dots into a single character. Always explicitly strip control characters using `Regex("[\\x00-\\x1F\\x7F]")`. Finally, safely `trim` the string of potentially dangerous bounds characters like `.` and `_` which might result from the previous replacements.
